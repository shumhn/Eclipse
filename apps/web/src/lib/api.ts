...
async function parseApiResponse<T>(
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        `Backend request failed (${res.status}). Make sure the API is running on http://localhost:3001.`
      );
    }
...
export async function fetchAgentStatus(): Promise<AgentStatus> {
  ...
  const json = await parseApiResponse<AgentStatus>(res, 'Failed to fetch agent status');
  return json.data!;
}
