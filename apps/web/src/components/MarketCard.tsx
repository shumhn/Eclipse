    ...
    <Link href={`/markets/${market.publicKey}`} className="block h-full group">
      ...
      `}>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2 text-xs font-medium text-poly-text-muted">
            <span>Politics</span>
          ...
          {positionsHidden && (
            <div className="flex items-center gap-1.5 text-[10px] text-poly-green font-semibold bg-poly-green/10 border border-poly-green/20 px-2.5 py-1 rounded-full shadow-[0_0_10px_rgba(43,168,89,0.1)]">
              <Shield className="w-3 h-3" /> TEE
            </div>
          )}
          {!positionsHidden && active && tradingEnabled && (
            <div className="flex items-center gap-1.5 text-[10px] text-poly-blue font-semibold bg-poly-blue/10 border border-poly-blue/20 px-2.5 py-1 rounded-full">
        ...
        {/* Odds Area */}
        {positionsHidden ? (
           <div className="flex items-center justify-between mt-auto bg-[#030608]/50 border border-white/5 rounded-xl px-4 py-3 group-hover:border-poly-green/20 transition-colors">
             <div className="text-poly-green text-sm font-medium flex items-center gap-2">
               <Shield className="h-4 w-4" /> 
               <span className="tracking-wide">Shielded Odds</span>
             </div>
             <div className="text-xs font-medium px-4 py-1.5 bg-white/5 border border-white/10 text-white rounded-lg group-hover:bg-poly-green group-hover:text-black group-hover:border-poly-green transition-all duration-300">
               Trade
             </div>
           </div>
        ) : (
          <div className="flex items-center gap-3 mt-auto">
            <div className="flex-1 flex flex-col items-center justify-center bg-poly-green/5 border border-poly-green/10 rounded-xl p-3 group-hover:bg-poly-green/10 group-hover:border-poly-green/30 transition-all duration-300">
              <div className="text-poly-green text-sm font-semibold tracking-wide">Yes {(prices.yes * 100).toFixed(0)}¢</div>
            </div>
            <div className="flex-1 flex flex-col items-center justify-center bg-poly-red/5 border border-poly-red/10 rounded-xl p-3 group-hover:bg-poly-red/10 group-hover:border-poly-red/30 transition-all duration-300">
              <div className="text-poly-red text-sm font-semibold tracking-wide">No {(prices.no * 100).toFixed(0)}¢</div>
            </div>
          </div>
        )}

        {/* Footer info (Volume) */}
        <div className="flex items-center justify-between text-xs text-poly-text-muted mt-5 pt-4 border-t border-white/5">
